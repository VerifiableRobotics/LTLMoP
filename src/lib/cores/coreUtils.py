...
def cnfToConjuncts(cnfIndices, mapping, cnfMapping):
    ...
    return conjuncts
    #clean conjuncts before returning
    #return cleanConjuncts(conjuncts, cnfMapping, cnfIndices, mapping)

def cleanConjuncts(conjuncts, cnfMapping, cnfIndices, mapping):
        #for each conjunct, check if the number of clauses that overlap with the other conjuncts are at least as many as the number of guilty clauses generated by that conjunct
        cleaned = [k for k in conjuncts if not (len(set([c for x in conjuncts for c in cnfMapping[x] if x!=k]).intersection(cnfMapping[k])) >= len(set(mapping[k]).intersection(cnfIndices)))]
        return cleaned
