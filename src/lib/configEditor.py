...
import project
import sys, os, re
from configClass import configClass, RobotObject
...
#        self.type = "robottype"
#        self.name = "robotname"
#        self.handlers = {"handlername":"handlerconfig(val)"}
#
#class config:
...
#        self.initial_truths = ["hide"]
#        self.sensor_mapping = {"propname":"robotname.sensor(arg) and robotname2.sensor2(arg2)"}
#        self.actuator_mapping = {}
#        self.robots = [robot()]

#class handlerConfig:
#    def __init__(self, htype, hname):
#
#    def __str__(self): 
#
...
class handlerConfigDialog(wx.Dialog):
    def __init__(self, parent, *args, **kwds):
        ...
        self.handlerLoader = parent.handlerLoader
    ...
    def _handler2dialog(self, handler):
        self.SetTitle("Configure %s" % handler['handlerName'])
        print handler
        drawParamConfigPane(self.panel_configs, handler['arg'])
        self.label_info.SetLabel(handler['comment'])
...
class simSetupDialog(wx.Dialog):
    def __init__(self, *args, **kwds):
        ...
        if not os.path.isfile(os.path.join(self.proj.project_root,'configs',current_config)):
            print "WARNING: Cannot find config file '%s' as referenced in spec file.  Ignoring." % current_config
    ...
    def onConfigImport(self, event): # wxGlade: simSetupDialog.<event_handler>
        ...
        # import the config file
        cfg = configClass(self.proj, fileName)        
        self.list_box_experiment_name.Append(cfg.name, cfg)
        self.list_box_experiment_name.Select(self.list_box_experiment_name.GetCount()-1)
        self._cfg2dialog(cfg)
    ...
    def onClickEditMapping(self, event): # wxGlade: simSetupDialog.<event_handler>
        ...
        obj = self._getSelectedConfigObject()
        dlg._mapping2dialog(obj.propMapping)
        if dlg.ShowModal() != wx.ID_CANCEL:
            obj.propMapping = dlg.mapping
        dlg.Destroy()
        event.Skip()
...
class addRobotDialog(wx.Dialog):
    def __init__(self, parent, *args, **kwds):
        ...
        self.handlerLoader = handlerLoader.handlerLoader(self.proj)
        self.handlerLoader.loadAllHandlers()
        
        for htype in self.robot.handlers.keys():
            self.handler_labels[htype] = wx.StaticText(self, -1, "%s:" % htype) 
            #self.handler_combos[htype] = wx.ComboBox(self, -1, choices=self.proj.loadHandlerNames(htype), style=wx.CB_DROPDOWN)
            self.handler_combos[htype] = wx.ComboBox(self, -1, choices=self.handlerLoader.getAvailableHandler(htype,"nao"), style=wx.CB_DROPDOWN)
        ...
        fileList = os.listdir(os.path.join(self.proj.ltlmop_root,'lib','handlers','robots'))

        for r in fileList:
            if os.path.isdir(os.path.join(self.proj.ltlmop_root,'lib','handlers','robots',r)):
                self.combo_box_robottype.Append(r)
                #if r == self.robot.type:
    ...
    def _robot2dialog(self, robot):
        ...
        for htype in self.robot.handlers.keys():
            # select the appropriate handler (stripping off any arguments), if specified
            # TODO: what should the correct behavior be if it doesn't exist in the list of handlers already?
            self.handler_combos[htype].SetStringSelection(self.robot.handlers[htype].split("(")[0])
    ...
    def onClickConfigure(self, event):
        ...
        for htype, b in self.handler_buttons.iteritems():
            if src is b:
                ...
                dlg._handler2dialog(self.handlerLoader.pose_handler[self.handler_combos[htype].GetValue()][0])
...
class propMappingDialog(wx.Dialog):
    ...
    def onClickMapping(self, event):
        ...
        if event.GetEventType() in [wx.EVT_KEY_DOWN, wx.EVT_KEY_UP] and \
           event.GetKeyCode() not in [wx.K_LEFT, wx.K_RIGHT, wx.K_UP, wx.K_DOWN, wx.K_HOME, wx.K_END, \
                                      wx.K_NUMPAD_LEFT, wx.K_NUMPAD_RIGHT, wx.K_NUMPAD_UP, wx.K_NUMPAD_DOWN]:
            event.Skip()
            return
    ...
    def onEditMapping(self, event): # wxGlade: propMappingDialog.<event_handler>
        ...
        self.mapping[prop_name] = event.GetString()
        event.Skip()
