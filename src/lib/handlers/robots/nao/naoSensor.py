...
class naoSensorHandler:
    ...
    def seeRedBall(self, consec, initial=False):
        """
        Check whether the nao sees a red ball
        
        consec (int): The number of consecutive detections to consider the ball
                        as seen
        """
        ...
        if initial:
            ...
            self.redBallData = {"pos":[], "seenCount":0}
            self.sensorData["redBallData"] = self.redBallData
            return True
        else:
            # Have seen the ball
            if self.trackerProxy.isNewData():
                pos = self.trackerProxy.getPosition()
                pos = [(x * 1000) for x in pos]
                pos[2] -= zOffset
                self.redBallData['pos'] = pos
                self.redBallData['seenCount'] += 1
                if self.redBallData['seenCount'] > consec:
                    return True
                else:
                    return False
            # Have not seen the ball
            else:
                self.redBallData["seenCount"] = 0
                return False
                
              
    def _getRedBallPose(self,initial=False):
        if not initial:
            return self.redBallData
