...
"""
    =============================================================
    RRTControllerHelper.py - Randomly Exploring Rapid Tree Controller
    =============================================================
...
def buildTree(p,theta,vert, R, system, regionPoly,nextRegionPoly,isFreespace,q_gBundle, last=False):
    ...
    if system == 1:    ## Nao
        ...
        timeStep = 10   #time step for calculation of x, y position
    ...
    if system == 1:    ## Nao
        ...
        velocity = 2    # what is used in RRTControllerHelper.setVelocity
    #############tune velocity OMEGA, TIME STEP
    ...
    while path == 0:
        ...
        #print "Helper 106: q_gBundle",q_gBundle
        while i < q_gBundle.shape[1]:   ###not sure about shape
            ...
            EdgePolyGoal    = Polygon((tuple(array(lowerEdgeG[:,0].T)[0]),tuple(array(upperEdgeG[:,0].T)[0]),tuple(array(upperEdgeG[:,1].T)[0]),tuple(array(lowerEdgeG[:,1].T)[0])))
            ...
            if connect_goal :
                ...
                BoundPolyPoints = asarray(pointList(BoundPoly))
        ...
        if path == 1:
            ...
            #print "Helper 224: numofPoints:", numOfPoint, "q_g",q_g
            if numOfPoint < 3:
                numOfPoint = 3
            x = linspace( V[1,shape(V)[1]-1], q_g[0,0], numOfPoint )
            y = linspace( V[2,shape(V)[1]-1], q_g[1,0], numOfPoint )
            #print "190:","x.shape[0]",x.shape[0],"numOfPoint",numOfPoint
        ...
        if path == 0:
            ...
            while success == 0 and hit_count <= 2:
                ...
                path_robot = Circle(radius,(xPrev,yPrev))
                while j <= timeStep:
                    xOrg      = xPrev
                    yOrg      = yPrev
                    ...
                    path_robot = path_robot + Circle(radius,(xPrev,yPrev))

                    j = j + 1

                path_all = convexHull(path_robot)
                #print "Keep checking path"
    ...
    plt.ylabel('y')
    BoundPolyPoints = asarray(pointList(BoundPoly))
    print  '266:BoundPolyPoints:', BoundPolyPoints
    #for i in range(BoundPolyPoints.shape[1]):
    plt.plot(BoundPolyPoints[:,0],BoundPolyPoints[:,1],'k')
    plt.plot(V[1,:],V[2,:],'b')
