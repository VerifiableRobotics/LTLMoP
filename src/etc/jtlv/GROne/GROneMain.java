	...
	public static void main(String[] args) throws Exception {
        ...
        if (synthesisSucceeded(g)) {
            ...
            if (opt_symbolic) {
                ...
                out_writer.write("#\n");
                out_writer.write("# This header contains information used by LTLMoP's BDDStrategy.\n");
                out_writer.write("# Currently, the only metadata is 1) the total number of system goals\n");
                out_writer.write("# and 2) the mapping between variable numbers and proposition names.\n");
                out_writer.write("# \n");
                out_writer.write("# Some special variables are also added:\n");
                out_writer.write("# \t- `_jx_b*` are used as a binary vector (b0 is LSB) to indicate\n");
                out_writer.write("# \t  the index of the currently-pursued goal.\n");
                out_writer.write("# \t- `strat_type` is a binary variable used to indicate whether we are\n");
                out_writer.write("# \t   moving closer to the current goal (0) or transitioning to the next goal (1)\n");
                out_writer.write("# \n");
                out_writer.write("# Num goals: " + g.sysJustNum + "\n");
